...
export default function GPTPage(): RC {
  ...
  useEffect(() => {
    import(/* webpackPreload: true */ 'react-markdown').then(setMdModule)
    import(/* webpackPreload: true */ 'highlight.js').then(setHlModule)
  }, [])
    ...
    gptMultipleChatApi(text, num)
      .then(answers => {
        ...
        if (Markdown && highlight) {
          setTimeout(() => {
            document
              .querySelectorAll<HTMLElement>('.pa-gpt__md pre code')
              .forEach(highlight.highlightElement)
          }, 100)
        }
      })
            ...
            <Space css={{ width: '100%' }} direction="vertical">
              ...
              >
                ...
                >
                  提出的问题
                  <Tooltip
                    content={`${formatDistanceToNow(new Date(record.timestamp), {
                      addSuffix: true,
                    })}`}
                  >
                    <ChatBubbleHelpIcon
                      css={{ margin: '0 4px', color: 'var(--td-gray-color-5)' }}
                    />
                  </Tooltip>
                  ：
                </div>
              ...
              >
                <Space css={{ width: '100%' }} direction="vertical" align="center">
                  <div className="pa-text" css={{ textAlign: 'center' }}>
                    {cursor <= 0
                      ? '当前回答'
                      : `第 ${records.length - cursor}/${records.length} 个`}
                  </div>
