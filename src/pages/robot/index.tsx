...
import { SK_ROBOT_CONFIG } from '@/utils/storageKeys'
...
const defaultRemeberConfig: IRemeberConfig = { isRemeber: false, tabType: 'wxbiz' }
...
export default function RobotPage(): RC {
  ...
  const submitMessageHandler = (result: SubmitContext<any>) => {
  ...
  return (
    <PageLayout>
      ...
      >
        ...
        {tab.type !== 'wxbiz' ? (
          ...
          >
            <Input
              ...
              onChange={v => void setRemeberConfig(t => ({ ...t!, secret: v }))}
            />
          </FormItem>
        ) : null}
        ...
        {tab.type !== 'feishu' && !isAtAll ? (
          <FormItem name="atList" label="@ 群成员">
            <TagInput
              placeholder="请输入需要 @ 的成员手机号，输入完请按 “Enter” 回车键"
              clearable
            />
          </FormItem>
        ) : null}
